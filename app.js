...
async function api(path, opts){
  ...
  catch { json = { error: text.slice(0, 180) }; }
  return { res, json };
}
...
async function refreshMe(){
  const { json } = await api("/api?path=auth/me");
...
function bindAuthUI(){
  ...
  $("#logoutBtn")?.addEventListener("click", async ()=>{
    await api("/api?path=auth/logout", { method:"POST" });
    await refreshMe();
  ...
  $("#doLogin")?.addEventListener("click", async ()=>{
    ...
    const { res, json } = await api("/api?path=auth/login", {
      method:"POST",
  ...
  $("#doRegister")?.addEventListener("click", async ()=>{
    ...
    if(!captchaToken){
      $("#registerMsg") && ($("#registerMsg").textContent = "Selesaikan verifikasi manusia dulu.");
      return;
    }

    const { res, json } = await api("/api?path=auth/register", {
      method:"POST",
      headers:{ "content-type":"application/json" },
      body: JSON.stringify({ email, password: pass, captchaToken })
    });

    // reset captcha biar bisa coba lagi
...
async function loadNovels(){
  const { res, json } = await api("/api?path=novels");
...
function renderPick(){
  ...
  for(const n of arr){
    ...
    el.querySelector("[data-like]")?.addEventListener("click", async ()=>{
      const r = await fetch(`/api?path=novels/${n.id}/like`, { method:"POST" });
      if(r.status === 401){
...
async function uploadCoverDataUrl(dataUrl, filename){
  const { res, json } = await api("/api?path=upload-cover", {
    method:"POST",
    headers:{ "content-type":"application/json" },
...
async function loadNovel(id){
  const { res, json } = await api(`/api?path=novels/${id}`);
